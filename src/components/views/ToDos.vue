...
const updateChecked = (todoId, checked) => {
  const todo = data.value.find((todo) => todo.id === todoId)
  if (todo) {
...
<template>
  <div class="container">
    ...
    <button @click="updatePoints(3)">larger</button> -->
      ...
      <!-- <div class="wrapper">
        <form @submit.prevent="addTask" class="form-item">
        ...
        <div class="buttons">
          ...
          <button type="button" class="buttons__btn done">
            <svg
              class="w-6 h-6 text-gray-800 dark:text-white"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
      ...
      <section class="main">
        ...
        <ToDoItem
          ...
          @update="updateChecked(todo.id, todo.checked)"
        />
      </section>
      <button class="btn-add" type="button">Add a New Task</button>
    </template>
  </div>
</template>
